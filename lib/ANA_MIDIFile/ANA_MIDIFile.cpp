/*
  ANA_MIDIFile.cpp - An Arduino library for processing Standard MIDI Files (SMF).
  Copyright (C) 2012 Marco Colli
  All rights reserved.

  See ANA_MIDIFile.h for complete comments
...
/**
 * \file
 * \brief Main file for the ANA_MIDIFile class implementation
 */

uint64_t ANA_MIDIFile::getAbsoluteMillis(void)
{
  return _absolute_micros / 1000 + 0xFFFFFFFFFFFFFFFF / 1000 * _absolute_micros_overflow_cnt;
}

void ANA_MIDIFile::initialise(void)
{
  ...
  _absolute_micros = 0;
  _absolute_micros_overflow_cnt = 0;
...
void ANA_MIDIFile::synchTracks(void)
{
...
ANA_MIDIFile::ANA_MIDIFile(void)
{
  initialise();
}

ANA_MIDIFile::~ANA_MIDIFile()
{
  close();
}

void ANA_MIDIFile::begin()
{
}

void ANA_MIDIFile::close()
...
void ANA_MIDIFile::setTempoAdjust(int16_t t)
{
  if ((t + _tempo) > 0)
    _tempoDelta = t;
  calcTickTime();
...
void ANA_MIDIFile::setTempo(uint16_t t)
{
  if ((_tempoDelta + t) > 0)
    _tempo = t;
  calcTickTime();
...
void ANA_MIDIFile::setTimeSignature(uint8_t n, uint8_t d)
{
  _timeSignature[0] = n;
  _timeSignature[1] = d;
  calcTickTime();
...
void ANA_MIDIFile::setTicksPerQuarterNote(uint16_t ticks)
{
  _ticksPerQuarterNote = ticks;
  calcTickTime();
}

void ANA_MIDIFile::setMicrosecondPerQuarterNote(uint32_t m)
...
void ANA_MIDIFile::calcTickTime(void)
...
void ANA_MIDIFile::pause(bool bMode)
...
void ANA_MIDIFile::restart(void)
...
uint16_t ANA_MIDIFile::tickClock(void)
...
*/
  _absolute_micros += _tickTime;
  return (1);
}

uint8_t ANA_MIDIFile::getNextEvent(void)
{
  uint16_t ticks;
  ...
  // micros overflow after about 70 mins
  uint64_t lastMicros = _absolute_micros;
  _absolute_micros += _tickTime * _next_event_in_n_ticks;
  if (_absolute_micros < lastMicros){
    _absolute_micros_overflow_cnt++;
    log_v("Micros overflow: %d", _absolute_micros_overflow_cnt);
    }
  //  log_v("\nNext Event in n ticks: %d", _next_event_in_n_ticks);
  return (processEvents(_next_event_in_n_ticks));
}

uint8_t ANA_MIDIFile::processEvents(uint16_t ticks)
{
  uint8_t n;
  uint16_t t, tt;
  uint16_t progress;
  ...
  for (uint8_t i = 0; i < _trackCount; i++)
  {
    ...
    for (n = 0; n < 100; n++)
    {
      t = _track[i].getNextEvent(this, (n == 0 ? ticks : 0));
      //   DUMP(" t= ",t);
      if (t > 0)
      {
        if (t < tt)
  ...
  if (tt < 0xFFFF)
  {
    _next_event_in_n_ticks = tt;
  }
  else
  {
    _next_event_in_n_ticks = 1;
  }
  progress /= _trackCount;
  DUMP(" ( ", progress);
  DUMPS("%)");
  return progress;
  //   log_v("\nNext Event in n ticks: %d", _next_event_in_n_ticks);
...
int ANA_MIDIFile::load(const char *fname)
...
// Return one of the E_* error codes
{
  ...
  _fd = SD.open(_fileName, FILE_READ);
  if (!_fd)
    return (E_NO_OPEN);
  ...
  _tempfile = SD.open("temp.sync", FILE_WRITE);
  if (!_tempfile)
    return (E_NO_OPEN);
