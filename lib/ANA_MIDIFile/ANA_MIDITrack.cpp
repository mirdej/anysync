/*
  ANA_MIDITrack.cpp - An Arduino library for processing Standard MIDI Files (SMF).
...
ANA_MFTrack::ANA_MFTrack(void)
{
  reset();
}

ANA_MFTrack::~ANA_MFTrack()
{
}

void ANA_MFTrack::close(void)
{
  reset();
}

uint32_t ANA_MFTrack::getLength(void)
// size of track in bytes
{
  return _length;
}

bool ANA_MFTrack::getEndOfTrack(void)
// true if end of track has been reached
{
  return _endOfTrack;
}

void ANA_MFTrack::syncTime(void)
{
  _elapsedTicks = 0;
}

void ANA_MFTrack::restart(void)
...
int16_t ANA_MFTrack::getNextEvent(ANA_MIDIFile *mf, uint16_t tickCount)
...
uint8_t ANA_MFTrack::percentDone(void)
{
  return (100 * _currOffset / _length);
}

void ANA_MFTrack::parseEvent(ANA_MIDIFile *mf)
// process the event from the physical file
{
  ...
  case 0xe0 ... 0xef:
  {
    ...
    // mf->_tempfile.printf("%d %02x %02x %02x\n",mf->getAbsoluteMillis(),_mev.data[0],_mev.data[1],_mev.data[2]);
    uint32_t t = mf->getAbsoluteMillis();
    for (int i = 0; i < 4; i++)
    {
      mf->_tempfile.write(t & 0xFF);
      t = t >> 8;
    }
    mf->_tempfile.write(_mev.data[0]);
    mf->_tempfile.write(_mev.data[1]);
    mf->_tempfile.write(_mev.data[2]);
#if !DUMP_DATA
...
int ANA_MFTrack::load(uint8_t trackId, ANA_MIDIFile *mf)
