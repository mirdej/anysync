/*   ANA_MIDIFILE based on MD_MIDIFILE by Marco Colli

adapted by Michael Egger 2023 foor Anamorphose Acoustique

--> replace SDFat with normal SD Library
--> parse file inton "sync" file with absolute millisecond positions of note and cc events
...
---------------------
The ANA_MIDIFile library uses the following additional libraries and may need additional hardware components
...
- Changed CLI example to use ANA_cmdProcessor.
...
- Renamed ANA_MIDIFile Library for consistency with other MajicDesigns libraries.
...
/**
 * \file
 * \brief Main header file for the ANA_MIDIFile library
 */
...
public:
   ...
   */
  int16_t getNextEvent(ANA_MIDIFile *mf, uint16_t tickCount);
   ...
   */
  int load(uint8_t trackId, ANA_MIDIFile *mf);
...
protected:
   ...
   * \return No return data.
   */
  void parseEvent(ANA_MIDIFile *mf);
...
/**
 * Core object for the ANA_MIDIFile library
 *
...
public:
   ...
   * of the ANA_MIDIFile object. The SDFs object should be initialized before calling this
   * method.
   *
   ...
   * \return No return data
   */
  uint8_t processEvents(uint16_t ticks);
...
protected:
  uint64_t _absolute_micros;
  uint8_t _absolute_micros_overflow_cnt;
  ...
  uint16_t _next_event_in_n_ticks;
  ...
  File _tempfile;
  File _syncfile;
  ANA_MFTrack _track[MIDI_MAX_TRACKS]; ///< the track data for this file
};
